...
module.exports = (authService = require('../services/authService')(), validationService = validator) => {
    const authController = express.Router();
    authController.post('/login', handleAsyncError(async (req, res, next) => {
        /*
            note: maybe create a request validator ???
        */
       validationService.validate(req.body, { email: [required()], password: [required()] });
